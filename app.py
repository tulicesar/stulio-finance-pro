...
st.markdown("""
    ...
    /* --- TABLAS XL PARA CELULAR (PEDIDO ESPECIAL) --- */
...
# --- 3. REPORTE PDF (COLOMBIA TIME + COLOR FIX) ---
...
if not st.session_state.autenticado:
    ...
    with col2:
        ...
        with tab_reg:
            rn = st.text_input("Nombre Completo"); ru = st.text_input("ID de Usuario"); rp = st.text_input("Pass", type="password")
            if st.button("Crear Cuenta"):
                if ru in db_u: st.error("El usuario ya existe.")
                else:
...
df_ed_g = st.data_editor(df_mes_g.reindex(columns=["Categoría", "Descripción", "Monto", "Valor Referencia", "Pagado"]).reset_index(drop=True), use_container_width=True, num_rows="dynamic", column_config={"Monto": config_moneda}, key="g_ed")
...
with inf1:
    t_df = df_ed_g.copy(); t_df['V'] = t_df.apply(lambda r: r['Monto'] if r['Pagado'] else 0, axis=1)
...
# Botón Guardar (2 espacios debajo)
