...
def cargar_bd():
    ...
    try:
        ...
        # Aseguramos que siempre sean números para evitar el error TypeError
...
def calcular_metricas(df_g, nom, otr, s_ant):
    ...
    # Copia para no dañar original y asegurar números
    temp = df_g.copy()
...
with st.sidebar:
    ...
    if not i_prev.empty:
        ...
        saldo_arrastrado = float(bf_pasado)
    ...
    s_in = st.number_input("Saldo Anterior", value=saldo_arrastrado if arrastrar else (float(d_act_i["SaldoAnterior"].iloc[0]) if not d_act_i.empty else 0.0), disabled=arrastrar)
...
# --- 🚀 REGISTRO (SIN CREACIÓN AUTOMÁTICA) ---
st.markdown("### 📝 Registro de Movimientos")
# Filtramos ÚNICAMENTE lo que existe en el Excel para este mes y usuario
...
# Editor de datos - Tulio, aquí tú agregas o borras a tu gusto
df_ed = st.data_editor(df_v, column_config=config_c, use_container_width=True, hide_index=True, num_rows="dynamic", key="editor_final_tulio")
...
if st.button("💾 GUARDAR CAMBIOS DEFINITIVOS"):
    # Filtramos filas vacías y asignamos datos de control
