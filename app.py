...
def generar_pdf_reporte(df_g_full, df_i_full, df_oi_full, meses, titulo, anio, u_id):
    ...
    for m in meses:
        ...
        # Detalle de ingresos
        c.setFillColor(colors.black); c.setFont("Helvetica-Bold", 9); c.drawString(60, y, "RESUMEN DE INGRESOS"); y -= 15
        c.setFont("Helvetica", 8); c.drawString(60, y, f"Saldo Ant: $ {s_ant:,.0f} | Nómina: $ {nom:,.0f} | Extras (Total): $ {otr_sum:,.0f}"); y -= 20
        
        if not oi_m.empty:
            c.setFont("Helvetica-Bold", 8); c.drawString(60, y, "DETALLE DE INGRESOS ADICIONALES"); y -= 12
            for _, r_oi in oi_m.iterrows():
                if y < 60: c.showPage(); y = head(c, titulo, anio); c.setFont("Helvetica", 8)
                c.setFont("Helvetica", 8); c.drawString(65, y, f"● {r_oi['Descripción']}"); c.drawRightString(480, y, f"$ {r_oi['Monto']:,.0f}"); y -= 10
            y -= 10
...
if not st.session_state.autenticado:
    ...
    with col2:
        ...
        with t2:
            rn = st.text_input("Nombre"); ru = st.text_input("ID"); rp = st.text_input("Contraseña", type="password")
            if st.button("Crear Cuenta"):
                db_u[ru] = {"pass": rp, "nombre": rn}; guardar_usuarios(db_u); st.success("Éxito")
    st.stop()
...
with st.sidebar:
    ...
    i_ant_r = df_i_full[(df_i_full["Periodo"]==m_ant)&(df_i_full["Año"]==a_ant)&(df_i_full["Usuario"]==u_id)]
    g_ant_f = df_g_full[(df_g_full["Periodo"]==m_ant)&(df_g_full["Año"]==a_ant)&(df_g_full["Usuario"]==u_id)]
    oi_ant_f = df_oi_full[(df_oi_full["Periodo"]==m_ant)&(df_oi_full["Año"]==a_ant)&(df_oi_full["Usuario"]==u_id)]
    
    s_sug = 0.0
    if not i_ant_r.empty:
        _, _, _, _, bf_a, _ = calcular_metricas(g_ant_f, i_ant_r["Nomina"].sum(), oi_ant_f["Monto"].sum(), i_ant_r["SaldoAnterior"].iloc[0])
        s_sug = float(bf_a)

    st.divider()
    arr_on = st.toggle(f"Arrastrar de {m_ant}", value=not i_ant_r.empty)
    ...
    # RESTAURADO: BALANCES PROYECTADOS
...
with inf2:
    ...
    rad = math.radians(180 - (v_cl / 100 * 180))
    x_p, y_p = 0.5, 0.42
    fig2 = go.Figure()
    ...
    fig2.add_shape(type="line", x0=x_p, y0=y_p, x1=x_p+0.32*math.cos(rad), y1=y_p+0.32*math.sin(rad), line=dict(color="#d4af37", width=5))
